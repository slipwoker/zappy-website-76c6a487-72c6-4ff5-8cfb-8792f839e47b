document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (required && value === '') {
        setFieldError(field, field.getAttribute('data-error-required') || 'This field is required.');
        return false;
      }
      ...
      if (type === 'email' || name.toLowerCase().includes('email')) {
        if (!isValidEmail(value)) {
          setFieldError(field, field.getAttribute('data-error-email') || 'Please enter a valid email address.');
          return false;
        }
      }
      ...
      if (type === 'tel' || name.toLowerCase().includes('phone') || name.toLowerCase().includes('tel')) {
        if (!isValidPhone(value)) {
          setFieldError(field, field.getAttribute('data-error-phone') || 'Please enter a valid phone number.');
          return false;
        }
      }

      if (minLength && value.length < parseInt(minLength, 10)) {
        setFieldError(field, field.getAttribute('data-error-minlength') || 'Input is too short.');
        return false;
      }

      if (maxLength && value.length > parseInt(maxLength, 10)) {
        setFieldError(field, field.getAttribute('data-error-maxlength') || 'Input is too long.');
        return false;
      }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '76c6a487-72c6-4ff5-8cfb-8792f839e47b',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '76c6a487-72c6-4ff5-8cfb-8792f839e47b';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '76c6a487-72c6-4ff5-8cfb-8792f839e47b';
